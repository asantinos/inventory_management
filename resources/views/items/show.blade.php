                ...
                <div id="options-menu" class="hidden origin-top-right absolute top-12 right-0 w-56 rounded-md shadow-lg bg-white dark:bg-gray-700 ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="py-1">
                        ...
                        <form action="{{ route('loans.update', ['loan' => $item->activeLoan()->id]) }}" method="POST" class="inline">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="block w-full px-4 py-2 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-100 ease-in-out">Return Loan</button>
                        </form>
                        @else
                        <a href="{{ route('loans.show', $activeLoan->id) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-100 ease-in-out">View Loan</a>
                        @endif
                        @else
                        <a href="{{ route('loans.create', ['item_id' => $item->id]) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-100 ease-in-out">Borrow</a>
                        @endif

                        <a href="{{ route('items.edit', $item->id) }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-100 ease-in-out">Edit</a>
